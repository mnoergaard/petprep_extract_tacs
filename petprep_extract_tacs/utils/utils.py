...
def avgwf_to_tacs(avgwf_file, ctab_file, json_file):
    ...
    avgwf_df = pd.read_csv(avgwf_file, 
                           delim_whitespace=True, 
                           header = None,
                           names=ctab_df['name'])
    ...
    avgwf_df.insert(1, 'frame_end', np.array(data['FrameTimesStart']) + np.array(data['FrameDuration']))
...
def gtm_to_tacs(in_file, json_file, gtm_stats, pvc_dir):
    ...
    gtm_stats = pd.read_csv(gtm_stats, 
                            header = None, 
                            delim_whitespace=True, 
                            usecols=[1,2,4], 
                            names=['index','name','volume_mm3'])
