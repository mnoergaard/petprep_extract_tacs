...
def test_collect_and_merge_tsvs():
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        unmerged_run_tsvs = {os.path.basename(tsv): pandas.read_csv(tsv, sep='\t') for tsv in unmerged_run_tsvs}

        # run collect_and_merge_tsvs on tempdir
        merged_tsvs = collect_and_merge_tsvs(bids_dir=tempdir, tsv_type='tacs', delete_extra_runs=False)
        ...
        unmerged_morphs = [unmerged_run_tsvs[tsv] for tsv in unmerged_run_tsvs if 'morph' in tsv]
        ...
        unmerged_dsegs = [unmerged_run_tsvs[tsv] for tsv in unmerged_run_tsvs if 'dseg' in tsv]
